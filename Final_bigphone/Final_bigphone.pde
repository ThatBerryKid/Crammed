...
void setup() {
  ...
  player = new Player(color(0,200,0),0,0,100,100,3);
  //obj's//
...
class Player{
 ...
 float rectH;
 float bounce;
 Player(color tColour, float tRectX, float tRectY, float tRectW, float tRectH, float tBounce){
   colour  = tColour;
   rectX   = tRectX;
   rectY   = tRectY;
   rectW   = tRectW;
...
void draw() {
  ...
  if (menu == 0 && start == 1) {
    ...
    if (mouseX>width/2-75 && mouseX<width/2+75 && mouseY>ground.yLine+50 && mouseY<ground.yLine+105) {
    fill(user.c);}
    else{
    fill(20, 205, 0);}
    rect(width/2-75, ground.yLine+50, 150, 55);
    
    if (mousePressed && mouseX>width/2-75 && mouseX<width/2+75 && mouseY>ground.yLine+50 && mouseY<ground.yLine+105) {
      gamePiece1 = new GamePiece(gamp1);
      gamePiece2 = new GamePiece2(gamp2);
      start = 0;
      game  = 1;
